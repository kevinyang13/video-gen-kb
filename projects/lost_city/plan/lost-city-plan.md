...
**Last updated**: 2026-09-25 (rift coda cut; shots renumbered to the cut order; LTX production settings in §3c; escape ending, shot 14 restaged as a landscape total collapse, creature enlarged after a motion-bleed failure; per-scene prompts in [[projects]])
...
Nine shots, numbered in the order they play. Old → new: s10→s3, s3→s4, s7→s5, s9→s6, s13→s7, s14→s9; s1, s2, s8 and s15a–c unchanged.
...
**Dropped**: the s15 rift coda — a crack tearing the sky with winged reptiles pouring out and circling the ruins; it was rendered and assembled (30 s, 4K, music) but cut on 2026-09-25 as a different film from the one this plan tells, and the media was deleted. The prompts survive in this plan's §5 history and in the log. Also the dismount / mount-up beat (old s11, s12) — klein rendered two creatures for every "rider beside the mount" phrasing across three seeds, and the story reads without it. Never rendered from the original design: the ¾-back on the rider's shoulders, the creature head in profile, and the stone-bridge crossing.

**Story order**: arrival and exploration (s1, s2, s4, s5) → life with the creature (s3, s8, s6) → the city falls and the rider escapes (s7, s9). **s7 is the only unrendered shot**, and without it the film cuts from the build-up straight to the aftermath.
