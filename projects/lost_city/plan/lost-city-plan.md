...
**Last updated**: 2026-09-25 (shots renumbered to the cut order; shot 15 rift coda; LTX production settings in §3c; escape ending, shot 14 restaged as a landscape total collapse, creature enlarged after a motion-bleed failure; per-scene prompts in [[projects]])
...
- **Action, not staging.** "left to right, camera tracks alongside" was ignored on s1; the same wording worked on s2 and s5. Write the action you want, accept the camera you get, or shoot that frame on Wan.
- **Close-ups: hold position.** Asking a close-up for locomotion (s4 v1) produced a full walk cycle that left the frame by 2 s *and* hallucinated a second rider behind. Use "stands still… only shifts its weight" and say the path behind **stays empty**.
- **Motion-stability rule**: slow in-place actions (walk, drink, weight shift, breathing) hold the creature design for the full 10 s; fast locomotion (gallop, jump) drifts toward a horse after ~4–5 s → trim on the first soft frame (s6 → 116 f).
- **Never write "lightning" for a sky rift.** klein renders a literal lightning bolt and ignores the tear. Torn-sky vocabulary works instead: *the sky is torn wide open by a long jagged crack like a sheet of glass broken from edge to edge, white light pouring through the gap, the torn edges glowing, clouds peeling back along them* (shot 15a, three seeds).
- **Motion bleeds onto small subjects.** In a shot where something else is destroyed, LTX applies those verbs to the subject too: s9 v1 sheared the creature's *head* off and dropped it like a falling spire. Fixes, in order of effect: make the subject large (≥ a third of the frame — a head that is ~30 px in the 32×32 latent gets re-synthesised as debris), name the subject and its stillness **before** any falling verb, use positive rigidity words (*solid, still, intact, head held level and attached, only the ribs move*), and add one clause pinning the destruction to the distance. Last resort: render the destruction as a plate with no creature in it.
...
## 6. Shot list (the cut, renumbered 2026-09-25)

Nine shots plus the rift coda, numbered in the order they play. Old → new: s10→s3, s3→s4, s7→s5, s9→s6, s13→s7, s14→s9; s1, s2, s8 and s15a–c unchanged.

| # | Shot | Camera | Subject motion | State |
|---|---|---|---|---|
| s1 | **Reference frame**: side, rider + creature walk toward the city | slow side tracking | walk, cloak, waterfall, birds | 4K |
| s2 | Extreme wide from a ledge: city skyline, tiny rider on the path | very slow push in | mist drift, birds, waterfall | 4K, shot twice (Wan and LTX) |
| s3 | **Drinking** — muzzle in a jungle pool below a waterfall | fixed | head lowers and lifts, ripples, rider looks around | 4K + music — the steadiest clip of the project |
| s4 | Low angle, creature's feet and forelegs on wet stone, ferns | fixed lens | feet plant, dust puffs, grass sway | 4K + music |
| s5 | Canyon: rider entering the spire canyon, god rays overhead | side tracking | walk, dust in the light, birds | 4K + music |
| s6 | **Jumping** — leap over a fallen pillar | tracking alongside | leap, landing, dust | clip only, trimmed to 4.6 s — drifts toward a horse after that |
| s7 | **Escape** — gallop down the avenue as a spire shears and falls behind | tracking alongside at speed | gallop, tumbling blocks, rolling dust wall | **not rendered** — prompt written; keep the creature large, trim on the first soft frame |
| s8 | **Running** — full gallop across a clearing | tracking alongside | gallop, dust, cloak streaming | 4K + music |
| s9 | **Escape, final shot** — wide landscape from a ridge: every spire falls until the skyline is empty | fixed, wide | creature solid and still in the near foreground, rider looks back; the city comes down on the horizon, last tower by ~8 s | 4K + music |
| s15a–c | **The rift coda** — a crack tears the sky, the flock pours out and circles the dead city | fixed · tilt down · fixed wide | see §5 | assembled: 30 s 4K with music |

**Dropped**: the dismount / mount-up beat (old s11, s12) — klein rendered two creatures for every "rider beside the mount" phrasing across three seeds, and the story reads without it. Never rendered from the original design: the ¾-back on the rider's shoulders, the creature head in profile, and the stone-bridge crossing.

**Story order**: arrival and exploration (s1, s2, s4, s5) → life with the creature (s3, s8, s6) → the city falls and the rider escapes (s7, s9) → the coda (s15). **s7 is the only unrendered shot**, and without it the film cuts from the build-up straight to the aftermath.
...
| s3 drinking | 2026-09-22 | **Best clip of the batch.** Fresh project (not a clone), still loaded from disk, LTX 1024×576 × 257 f. Creature design holds for the whole 10.3 s — head lowers and lifts, water drips, ripples settle, rider looks around. Confirms the rule: **slow, in-place actions are stable; fast locomotion drifts**. `s3_ltx_v1_4k_music.mp4`. |
