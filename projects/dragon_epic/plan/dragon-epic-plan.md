...
| E1 | 2026-09-21 | **No adapter for klein.** Control dropdown, *Comp.* tab with FLUX.2 klein selected = empty (Official and Community both). *All* tab has PuLID 0.9.1 (FLUX.1 dev only), FLEX.1 Redux SigLIP2 (FLUX.1), IP-Adapter Plus Face (SD1.5/SDXL), IP-Adapter FaceID (Kwai Kolors) — none load on klein. Face path for klein = **Control Inputs → Moodboard** (klein's native reference-image input; loaded `raw/face/hero_face_head.jpg` at weight 100%). Draw Things gotchas: dragging a file onto the *canvas* instead of the Moodboard area loads it as the base image and auto-captions it into the prompt ("1girl, aqua eyes…") — restart the project if that happens; new projects inherit the last-used model (Wan), so re-pick FLUX.2 klein and re-set 1280×768 each time. |
