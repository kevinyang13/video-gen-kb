...
**Last updated**: 2026-09-22 (LTX production settings in §3c; escape ending, shot 14 restaged as a landscape total collapse; per-scene prompts in [[projects]])
...
| 14 | **Escape / final shot** — extreme wide landscape from a ridge: every spire falls until the skyline is empty | fixed, extreme wide | creature still and breathing in the foreground, rider looks back; the whole city comes down in the distance, last tower by ~8 s, ends on a dust skyline | city (creature is scale only) |
